...
const seed={
  ...
  cloud:{emailHint:'joeyoliveira8@gmail.com',clientId:'',folderName:'Meu Caixa - Backups',autoBackup:false,lastBackupAt:null,lastFileId:null},
...
document.querySelector('#connectDrive').onclick=async()=>{try{await requestDriveToken();renderData()}catch(e){alert(e.message)}};
...
async function backupToDrive(silent=false){
  ...
  try{const folderId=await findOrCreateDriveFolder();let file=await findBackupFile(folderId);const body=JSON.stringify({...state,exportedAt:new Date().toISOString()},null,2);if(!file){let r=await driveFetch('https://www.googleapis.com/drive/v3/files?fields=id,name,modifiedTime',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:'meu-caixa-backup.json',mimeType:'application/json',parents:[folderId]})});file=await r.json()}await driveFetch(`https://www.googleapis.com/upload/drive/v3/files/${file.id}?uploadType=media`,{method:'PATCH',headers:{'Content-Type':'application/json'},body});state.cloud.lastBackupAt=new Date().toISOString();state.cloud.lastFileId=file.id;save({backupOld:false,cloud:false});updateDriveStatus(`Backup enviado ao Drive em ${new Date(state.cloud.lastBackupAt).toLocaleString('pt-BR')}.`);if(!silent)alert('Backup enviado ao Google Drive.')}catch(e){if(!silent)alert(e.message);throw e}
}
document.querySelector('#backupDrive').onclick=()=>backupToDrive(false).catch(()=>{});
document.querySelector('#restoreDrive').onclick=async()=>{try{if(!driveToken)await requestDriveToken();const folderId=await findOrCreateDriveFolder(),file=await findBackupFile(folderId);if(!file){alert('Nenhum backup do Meu Caixa foi encontrado nessa pasta.');return}if(!confirm(`Restaurar o backup do Drive${file.modifiedTime?` de ${new Date(file.modifiedTime).toLocaleString('pt-BR')}`:''}? O estado atual será mantido como cópia local anterior.`))return;const r=await driveFetch(`https://www.googleapis.com/drive/v3/files/${file.id}?alt=media`),data=migrateState(await r.json());localStorage.setItem(PREVIOUS_KEY,JSON.stringify(state));state=data;state.cloud={...seed.cloud,...state.cloud,clientId:document.querySelector('#googleClientId').value.trim()||state.cloud.clientId,emailHint:document.querySelector('#googleEmailHint').value.trim()||state.cloud.emailHint};save({backupOld:false,cloud:false});renderAll();alert('Backup do Drive restaurado.')}catch(e){alert(e.message)}};
