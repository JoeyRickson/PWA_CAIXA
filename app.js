// SaldoPlan v2.5.2 — simulação calibrada com o holerite de setembro/2026.
...
const money = v => {
    ...
    if (comma >= 0 && dot >= 0) {
        ...
        if ((s.match(/\./g) || []).length > 1 || decimals > 2)
            s = s.replace(/\./g, '');
    }
    const n = Number(s);
...
function migrateState(raw) {
    ...
    if (migrated.cloud.folderName === 'Meu Caixa - Backups')
        migrated.cloud.folderName = 'SaldoPlan - Backups';
    ...
    if (typeof migrated.savingGoal !== 'number')
        migrated.savingGoal = money(migrated.savingGoal) || 500;
    return migrated;
}
function load() {
    try {
...
function save({ backupOld = true, cloud = true } = {}) {
    try {
        if (backupOld) {
            ...
            if (old)
                localStorage.setItem(PREVIOUS_KEY, old);
        }
        localStorage.setItem(STORAGE_KEY, JSON.stringify(state));
        localStorage.setItem(`${STORAGE_KEY}.savedAt`, new Date().toISOString());
        if (cloud && state.cloud?.autoBackup && (driveToken || cloudToken))
            scheduleCloudBackup();
    }
    catch (e) {
        console.error(e);
...
function updateLocalSaveStatus() { const el = document.querySelector('#localSaveStatus'); if (!el)
    return; const savedAt = localStorage.getItem(`${STORAGE_KEY}.savedAt`); el.textContent = savedAt ? `Última gravação neste aparelho: ${new Date(savedAt).toLocaleString('pt-BR')}.` : 'As alterações também são salvas automaticamente ao usar o app.'; }
...
function formatDate(s) { if (!s)
    return ''; const [y, m, d] = s.split('-'); return `${d}/${m}/${y}`; }
function setMoney(id, v) { const el = document.getElementById(id); if (!el)
    return; el.textContent = fmt.format(v); el.classList.toggle('negative', v < 0); }
...
function ensureRecurring() {
    ...
    state.fixedTemplates.forEach(t => {
        ...
        if (state.transactions.some(x => x.recurringMarker === marker))
            return;
...
function renderDashboard() {
    ...
    setMoney('cashNow', m.cashNow);
    setMoney('cashProjected', m.cashProjected);
    setMoney('flashNow', m.flashNow);
    document.querySelector('#flashProjected').textContent = `Projetado: ${fmt.format(m.flashProjected)}`;
    document.querySelectorAll('[data-bonus-label]').forEach(el => el.textContent = bonusLabel());
    document.querySelectorAll('[data-bonus-visibility]').forEach(el => el.classList.toggle('hidden', state.settings.bonusEnabled === false));
    setMoney('savedTotal', s.balance);
    document.querySelector('#savedMonthText').textContent = `No mês: ${fmt.format(s.monthNet)}`;
    const potential = Math.max(0, m.cashProjected);
    setMoney('savePotential', potential);
    document.querySelector('#goalText').textContent = `Meta mensal: ${fmt.format(state.savingGoal || 0)}`;
    document.querySelector('#goalProgress').style.width = `${state.savingGoal ? Math.min(100, potential / state.savingGoal * 100) : 0}%`;
    setMoney('incomeFirst', sum(m.inc.filter(x => x.incomeType === 'first')));
    setMoney('incomeSecond', sum(m.inc.filter(x => x.incomeType === 'second')));
    setMoney('incomeExtra', sum(m.inc.filter(x => x.incomeType === 'extra')));
    setMoney('incomeFlash', sum(m.inc.filter(x => x.incomeType === 'flash')));
    setMoney('fixedTotal', sum(m.exp.filter(x => x.expenseType === 'fixed')));
    setMoney('variableTotal', sum(m.exp.filter(x => x.expenseType === 'variable')));
    setMoney('unplannedTotal', sum(m.exp.filter(x => x.expenseType === 'unplanned')));
    renderBars(m.exp);
    renderPending(m.exp);
    document.querySelector('#importNote').classList.toggle('hidden', !state.importNote);
}
function renderBars(exp) {
    const by = {};
    exp.forEach(x => by[x.category || 'Sem categoria'] = (by[x.category || 'Sem categoria'] || 0) + Number(x.amount || 0));
...
function renderTransactions() { let tx = transactions(); if (filter === 'income')
    tx = tx.filter(x => x.kind === 'income'); if (filter === 'expense')
    tx = tx.filter(x => x.kind === 'expense'); if (filter === 'pending')
    tx = tx.filter(x => x.status === 'planned'); document.querySelector('#transactionList').innerHTML = tx.length ? tx.map(itemHtml).join('') : '<div class="empty">Nada por aqui.</div>'; }
...
function renderSavingChart(items) {
    const root = document.querySelector('#savingChart');
    if (!items.length) {
        root.innerHTML = '<div class="empty" style="width:100%">Registre seu primeiro aporte para começar o histórico.</div>';
        return;
    }
    let balance = 0;
    const pts = items.map((x, i) => { balance += x.type === 'deposit' ? Number(x.amount) : -Number(x.amount); return { i, balance, date: x.date }; });
    const vals = pts.map(p => p.balance), min = Math.min(0, ...vals), max = Math.max(1, ...vals), range = Math.max(1, max - min);
    const w = 620, h = 170, pad = 22;
...
function calculateExtraRecord(r) {
    let start = timeToMinutes(r.start), end = timeToMinutes(r.end);
    if (start === null || end === null)
        return { minutes: 0, nightMinutes: 0, hours50: 0, hours100: 0 };
    if (end <= start)
        end += 1440;
    let lunchStart = null, lunchEnd = null, lunchMinutes = 0;
    if (r.deductLunch && r.lunchStart && r.lunchEnd) {
        lunchStart = timeToMinutes(r.lunchStart);
        lunchEnd = timeToMinutes(r.lunchEnd);
        if (lunchEnd <= lunchStart)
            lunchEnd += 1440;
        while (lunchStart < start) {
            lunchStart += 1440;
            lunchEnd += 1440;
        }
        lunchMinutes = overlap(start, end, lunchStart, lunchEnd);
    }
    ...
    if (lunchStart !== null)
        night -= windows.reduce((s, [a, b]) => s + overlap(lunchStart, lunchEnd, a, b), 0);
    night = Math.max(0, night);
    const d = new Date(`${r.date}T12:00:00`);
    const is100 = !!r.holiday || d.getDay() === 0;
...
function irrf2026(gross, inss) {
    const simplified = 607.20;
    const deduction = Math.max(inss, simplified);
    const base = Math.max(0, gross - deduction);
    let before = 0;
    if (base <= 2428.80)
        before = 0;
    else if (base <= 2826.65)
        before = base * .075 - 182.16;
    else if (base <= 3751.05)
        before = base * .15 - 394.16;
    else if (base <= 4664.68)
        before = base * .225 - 675.49;
    else
        before = base * .275 - 908.73;
    before = Math.max(0, round2(before));
    let reduction = 0;
    if (gross <= 5000)
        reduction = Math.min(before, 312.89);
    else if (gross <= 7350)
        reduction = Math.max(0, round2(978.62 - 0.133145 * gross));
    reduction = Math.min(before, reduction);
    return { base: round2(base), before, reduction: round2(reduction), value: round2(Math.max(0, before - reduction)), deductionUsed: round2(deduction) };
}
function payrollMonthConfig(k = simKey()) { if (!state.payrollMonths[k])
    state.payrollMonths[k] = { referenceHours: null }; return state.payrollMonths[k]; }
function payrollSimulation() {
    const cfg = state.payrollConfig;
    const k = simKey();
    const monthCfg = payrollMonthConfig(k);
    const logs = state.overtimeLogs.filter(x => payrollCompetenceKey(x.date) === k);
    let h50 = 0;
    let h100 = 0;
    let night = 0;
    logs.forEach(r => {
        const c = calculateExtraRecord(r);
        h50 += c.hours50;
        h100 += c.hours100;
        night += c.nightHours;
    });
    if (monthCfg.referenceHours && logs.length === 0) {
        h50 = Number(monthCfg.referenceHours.h50 || 0);
        h100 = Number(monthCfg.referenceHours.h100 || 0);
        night = Number(monthCfg.referenceHours.night || 0);
    }
    const hourBase = Number(cfg.baseSalary || 0) / Math.max(1, Number(cfg.monthlyHours || 220));
    const v50 = round2(h50 * hourBase * 1.5);
    const v100 = round2(h100 * hourBase * 2);
    const nightValue = round2(night * hourBase * (Number(cfg.nightPct || 20) / 100));
    const extraBase = round2(v50 + v100 + nightValue);
    const calendar = getDSRCalendar(k);
    // Calibração baseada no holerite de setembro/2026:
    // DSR sobre HE/adicional noturno = extras / dias não-domingo * domingos.
    // Feriados não acrescentam DSR nessa regra de folha.
    const restDays = calendar.sundays;
    const workDays = Math.max(1, calendar.totalDays - calendar.sundays);
    const dsr = round2(extraBase / workDays * restDays);
    const extras = round2(extraBase + dsr);
    const gross = round2(Number(cfg.baseSalary || 0) + extras);
    const inss = inss2026(gross);
    const ir = irrf2026(gross, inss);
    const netMonth = round2(gross - inss - ir.value);
    const advance = round2(Number(cfg.advance || 0));
    const finalPay = round2(netMonth - advance);
    return {
        k,
        h50,
        h100,
        ...
        finalPay,
        logs,
        reference: monthCfg.referenceHours && logs.length === 0
    };
}
function renderSimulator() {
    document.querySelector('#simMonthLabel').textContent = `${MONTHS[simCursor.getMonth()]} ${simCursor.getFullYear()}`;
    const cfg = state.payrollConfig;
    document.querySelector('#simBaseSalary').value = Number(cfg.baseSalary).toFixed(2).replace('.', ',');
    document.querySelector('#simMonthlyHours').value = cfg.monthlyHours;
    document.querySelector('#simAdvance').value = Number(cfg.advance).toFixed(2).replace('.', ',');
    document.querySelector('#simNightPct').value = cfg.nightPct;
    document.querySelector('#simExtraGoal').value = cfg.extraGoal || 0;
    const holidayInput = document.querySelector('#simHolidayCount');
    if (holidayInput) {
        holidayInput.value = 0;
        const holidayLabel = holidayInput.closest('label');
        if (holidayLabel)
            holidayLabel.classList.add('hidden');
    }
    const referenceButton = document.querySelector('#loadJulyReference');
    if (referenceButton)
        referenceButton.textContent = 'Referência set/26';
    const p = payrollSimulation();
    document.querySelector('#simHours50').textContent = hoursLabel(p.h50);
    document.querySelector('#simValue50').textContent = fmt.format(p.v50);
    document.querySelector('#simHours100').textContent = hoursLabel(p.h100);
    document.querySelector('#simValue100').textContent = fmt.format(p.v100);
    document.querySelector('#simNightHours').textContent = hoursLabel(p.night);
    document.querySelector('#simNightValue').textContent = fmt.format(p.nightValue);
    setMoney('simDsrValue', p.dsr);
    document.querySelector('#simDsrDetail').textContent = `${p.restDays} DSR / ${p.workDays} dias no divisor`;
    setMoney('payBase', state.payrollConfig.baseSalary);
    setMoney('payExtras', p.extras);
    setMoney('payGross', p.gross);
    document.querySelector('#payInss').textContent = `− ${fmt.format(p.inss)}`;
    document.querySelector('#payIrrf').textContent = `− ${fmt.format(p.ir.value)}`;
    setMoney('payNetMonth', p.netMonth);
    document.querySelector('#payAdvance').textContent = `− ${fmt.format(p.advance)}`;
    setMoney('payFinal', p.finalPay);
    document.querySelector('#payrollReferenceNote').textContent = p.reference
        ? 'Referência de setembro/2026 carregada: 60h00 de HE 50%, 0h00 de HE 100% e 6h00 de adicional noturno. A simulação foi calibrada para reproduzir o holerite de setembro/2026.'
        : 'INSS e IRRF usam as tabelas de 2026. O DSR segue a regra observada no holerite de setembro/2026: somente domingos entram no cálculo; feriados não acrescentam DSR.';
    ...
    if (p.reference) {
      ...
      <div class="item">
        ...
        <span class="item-main">
          <strong>Referência do contracheque — setembro/2026</strong>
          <span>60h00 HE50 · 0h00 HE100 · 6h00 noturno</span>
        </span>
        <span class="item-side">
          <strong>${fmt.format(p.extras)}</strong>
          <span>Calibração</span>
        </span>
      </div>`;
    }
    document.querySelector('#extraList').innerHTML = refHtml + (records.length
        ? records.map(r => {
            const c = calculateExtraRecord(r);
            return `
          <button class="item" data-extra-edit="${r.id}" style="width:100%;text-align:left;cursor:pointer">
            <span class="item-icon">⌁</span>
            <span class="item-main">
              <strong>${escapeHtml(r.description)}</strong>
              <span>${formatDate(r.date)} · ${r.start}–${r.end}${r.holiday ? ' · HE 100%' : ''}</span>
            </span>
            <span class="item-side">
              <strong>${hoursLabel(c.minutes / 60)}</strong>
              <span>${c.is100 ? '100%' : '50%'}${c.nightHours ? ` · ${hoursLabel(c.nightHours)} not.` : ''}</span>
            </span>
          </button>`;
        }).join('')
        : '');
    if (!p.reference && !records.length) {
        document.querySelector('#extraList').innerHTML = '<div class="empty">Nenhuma hora extra registrada nesta competência.</div>';
    }
}
function renderData() {
    document.querySelector('#savingGoal').value = (state.savingGoal || 0).toFixed(2).replace('.', ',');
    document.querySelector('#bonusEnabled').checked = state.settings.bonusEnabled !== false;
    document.querySelector('#bonusName').value = state.settings.bonusName || 'Bônus';
    document.querySelector('#incomeType option[value="flash"]').textContent = bonusLabel();
    document.querySelector('#paySource option[value="flash"]').textContent = bonusLabel();
    document.querySelector('#cloudProvider').value = state.cloud.provider || 'google';
    document.querySelector('#googleEmailHint').value = state.cloud.emailHint || '';
    document.querySelector('#googleClientId').value = state.cloud.clientId || '';
...
function updateCloudProviderFields() {
    ...
    document.querySelector('#driveFolderName').closest('label').querySelector('span')?.remove();
    document.querySelector('#driveFolderName').closest('label').firstChild.textContent = isGoogle ? 'Pasta no Drive' : 'Pasta de backup';
    document.querySelector('#cloudAccountLabel').firstChild.textContent = `Conta ${name}`;
    document.querySelector('#cloudAccount').placeholder = provider === 'icloud' ? 'seu Apple ID' : 'voce@exemplo.com';
    ...
    button.textContent = `Conectar ao ${name}`;
...
function applyTheme() {
    ...
    document.querySelector('meta[name="theme-color"]').setAttribute('content', theme === 'dark' ? '#070a0f' : '#111827');
    document.querySelector('#themeQuickBtn').textContent = theme === 'dark' ? '☀' : '☾';
}
function toggleTheme() { state.settings.theme = state.settings.theme === 'dark' ? 'light' : 'dark'; save(); applyTheme(); renderData(); }
function go(view) { document.querySelectorAll('.view').forEach(v => v.classList.remove('active')); document.querySelector(`#view-${view}`).classList.add('active'); document.querySelectorAll('.nav-item').forEach(b => b.classList.toggle('active', b.dataset.nav === view)); if (view === 'transactions')
    renderTransactions(); if (view === 'savings')
    renderSavings(); if (view === 'simulator')
    renderSimulator(); if (view === 'fixed')
    renderFixed(); if (view === 'data')
    renderData(); window.scrollTo({ top: 0, behavior: 'smooth' }); }
// Navegação e atalhos
addEventListener('click', e => {
    ...
    if (action) {
        if (action.dataset.action === 'new-income')
            openEntry('income');
        else if (action.dataset.action === 'new-fixed')
            openTemplate();
        else if (action.dataset.action === 'new-saving')
            openSaving();
        else if (action.dataset.action === 'new-extra')
            openExtra();
        else
            openEntry('expense');
        return;
    ...
    if (edit) {
        const t = state.transactions.find(x => x.id === edit.dataset.edit);
        if (t)
            openEntry(t.kind, t);
        return;
    }
    const templ = e.target.closest('[data-template]');
    if (templ) {
        openTemplate(state.fixedTemplates.find(t => t.id === templ.dataset.template));
        return;
    }
    const sedit = e.target.closest('[data-saving-edit]');
    if (sedit) {
        openSaving(state.savings.find(x => x.id === sedit.dataset.savingEdit));
        return;
    }
    const xedit = e.target.closest('[data-extra-edit]');
    if (xedit) {
        openExtra(state.overtimeLogs.find(x => x.id === xedit.dataset.extraEdit));
        return;
    }
});
...
function openEntry(kind = 'expense', obj = null) {
    ...
    document.querySelector('#repeatMonthly').checked = !!obj?.templateId;
    if (kind === 'income') {
    ...
    document.querySelector('#amount').value = obj ? Number(obj.amount).toFixed(2).replace('.', ',') : '';
    dlg.showModal();
}
function openTemplate(t = null) {
    ...
    document.querySelector('#amount').value = t ? Number(t.amount).toFixed(2).replace('.', ',') : '';
    const day = t?.dueDay || 15;
    document.querySelector('#entryDate').value = `${cursor.getFullYear()}-${String(cursor.getMonth() + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
...
form.addEventListener('submit', e => {
    ...
    if (!description)
        description = kind === 'income' ? defaultIncomeDescription(incomeType) : defaultExpenseDescription(expenseType, category);
    if (!(amount > 0)) {
    ...
    if (templateMode) {
        ...
        if (ix >= 0)
            state.fixedTemplates[ix] = t;
        else
            state.fixedTemplates.push(t);
    }
    else {
...
document.querySelector('#incomeType').addEventListener('change', e => {
    if (document.querySelector('#entryKind').value !== 'income')
        return;
    const d = document.querySelector('#description'), autoValues = ['Salário - quinzena', 'Salário - final do mês', 'Entrada extra', 'Crédito Flash', ''];
    if (autoValues.includes(d.value.trim()))
        d.value = defaultIncomeDescription(e.target.value);
});
document.querySelector('#deleteEntry').onclick = () => { const id = document.querySelector('#entryId').value, templateMode = document.querySelector('#fixedTemplateMode').value === '1'; if (!id)
    return; if (!confirm('Excluir este item?'))
    return; if (templateMode) {
    state.fixedTemplates = state.fixedTemplates.filter(x => x.id !== id);
    state.transactions = state.transactions.filter(x => x.templateId !== id);
}
else
    state.transactions = state.transactions.filter(x => x.id !== id); save(); closeDialog(); renderAll(); };
// Guardado
...
savingForm.addEventListener('submit', e => { e.preventDefault(); const id = document.querySelector('#savingId').value, obj = { id: id || uid(), type: document.querySelector('#savingType').value, date: document.querySelector('#savingDate').value, account: document.querySelector('#savingAccount').value.trim() || 'Reserva', description: document.querySelector('#savingDescription').value.trim(), amount: money(document.querySelector('#savingAmount').value) }; if (!obj.date || !obj.description || !obj.amount)
    return; const ix = state.savings.findIndex(x => x.id === id); if (ix >= 0)
    state.savings[ix] = obj;
else
    state.savings.push(obj); save(); savingDlg.close(); renderAll(); });
document.querySelector('#deleteSaving').onclick = () => { const id = document.querySelector('#savingId').value; if (!id || !confirm('Excluir esta movimentação?'))
    return; state.savings = state.savings.filter(x => x.id !== id); save(); savingDlg.close(); renderAll(); };
// Horas extras
const extraDlg = document.querySelector('#extraDialog'), extraForm = document.querySelector('#extraForm');
function openExtra(obj = null) {
    const monthCfg = payrollMonthConfig();
    if (!obj && monthCfg.referenceHours && state.overtimeLogs.filter(x => payrollCompetenceKey(x.date) === simKey()).length === 0) {
        if (!confirm('Esta competência está usando a referência do contracheque. Ao adicionar um registro real, a referência será removida. Continuar?'))
            return;
        monthCfg.referenceHours = null;
        save();
    }
    extraForm.reset();
    document.querySelector('#extraId').value = obj?.id || '';
    document.querySelector('#extraFormTitle').textContent = obj ? 'Editar registro' : 'Novo registro';
    document.querySelector('#extraDate').value = obj?.date || baseDate(simCursor);
    document.querySelector('#extraDescription').value = obj?.description || '';
    document.querySelector('#extraStart').value = obj?.start || '17:18';
    document.querySelector('#extraEnd').value = obj?.end || '19:18';
    document.querySelector('#extraDeductLunch').checked = !!obj?.deductLunch;
    document.querySelector('#extraLunchStart').value = obj?.lunchStart || '11:00';
    document.querySelector('#extraLunchEnd').value = obj?.lunchEnd || '12:00';
    document.querySelector('#extraHoliday').checked = !!obj?.holiday;
    document.querySelector('#lunchFields').classList.toggle('hidden', !obj?.deductLunch);
    document.querySelector('#deleteExtra').classList.toggle('hidden', !obj);
    updateExtraPreview();
    extraDlg.showModal();
}
...
['#extraDate', '#extraStart', '#extraEnd', '#extraLunchStart', '#extraLunchEnd', '#extraHoliday'].forEach(sel => document.querySelector(sel).addEventListener('input', updateExtraPreview));
document.querySelector('#extraDeductLunch').addEventListener('change', e => { document.querySelector('#lunchFields').classList.toggle('hidden', !e.target.checked); updateExtraPreview(); });
document.querySelector('#closeExtraDialog').onclick = () => extraDlg.close();
extraForm.addEventListener('submit', e => { e.preventDefault(); const id = document.querySelector('#extraId').value, obj = { id: id || uid(), date: document.querySelector('#extraDate').value, description: document.querySelector('#extraDescription').value.trim(), start: document.querySelector('#extraStart').value, end: document.querySelector('#extraEnd').value, deductLunch: document.querySelector('#extraDeductLunch').checked, lunchStart: document.querySelector('#extraLunchStart').value, lunchEnd: document.querySelector('#extraLunchEnd').value, holiday: document.querySelector('#extraHoliday').checked }; const c = calculateExtraRecord(obj); if (!obj.date || !obj.description || !obj.start || !obj.end || c.minutes <= 0)
    return; const ix = state.overtimeLogs.findIndex(x => x.id === id); if (ix >= 0)
    state.overtimeLogs[ix] = obj;
else
    state.overtimeLogs.push(obj); save(); extraDlg.close(); renderAll(); });
document.querySelector('#deleteExtra').onclick = () => { const id = document.querySelector('#extraId').value; if (!id || !confirm('Excluir este registro de hora extra?'))
    return; state.overtimeLogs = state.overtimeLogs.filter(x => x.id !== id); save(); extraDlg.close(); renderAll(); };
document.querySelector('#savePayrollConfig').onclick = () => {
    state.payrollConfig.baseSalary = money(document.querySelector('#simBaseSalary').value);
    state.payrollConfig.monthlyHours = Math.max(1, decimal(document.querySelector('#simMonthlyHours').value));
    state.payrollConfig.advance = money(document.querySelector('#simAdvance').value);
    state.payrollConfig.nightPct = Math.max(0, decimal(document.querySelector('#simNightPct').value));
    state.payrollConfig.extraGoal = Math.max(0, decimal(document.querySelector('#simExtraGoal').value));
    save();
    renderSimulator();
    alert('Parâmetros da simulação salvos.');
};
document.querySelector('#loadJulyReference').onclick = () => {
    simCursor = new Date(2026, 8, 1);
    const m = payrollMonthConfig('2026-09');
    m.referenceHours = { h50: 60, h100: 0, night: 6 };
    state.payrollConfig.baseSalary = 5000;
    state.payrollConfig.monthlyHours = 220;
    state.payrollConfig.advance = 2000;
    state.payrollConfig.nightPct = 20;
    save();
    renderSimulator();
};
document.querySelector('#useSimulationIncome').onclick = () => { const p = payrollSimulation(), [y, m] = p.k.split('-').map(Number), last = new Date(y, m, 0).getDate(), date = `${y}-${String(m).padStart(2, '0')}-${String(last).padStart(2, '0')}`; let x = state.transactions.find(t => t.source === 'payroll-simulation' && t.date?.slice(0, 7) === p.k); const obj = { id: x?.id || uid(), kind: 'income', incomeType: 'second', description: 'Salário - final do mês (simulação)', amount: Math.max(0, p.finalPay), date, status: 'planned', source: 'payroll-simulation' }; if (x)
    ...
    state.transactions.push(obj); cursor = new Date(y, m - 1, 1); save(); renderAll(); alert('Entrada prevista atualizada em Lançamentos.'); };
...
function exportPdf() {
    ...
    const totals = metrics(), saved = savingsMetrics(), popup = window.open('', '_blank', 'width=1000,height=800');
    if (!popup) {
...
document.querySelector('#importJson').onchange = async (e) => { const f = e.target.files?.[0]; if (!f)
    return; operationStart('Importando informações', 'Lendo o arquivo e restaurando seus dados…'); try {
    const data = migrateState(JSON.parse(await f.text()));
    if (!Array.isArray(data.transactions))
        throw new Error();
    localStorage.setItem(PREVIOUS_KEY, JSON.stringify(state));
    state = data;
    save({ backupOld: false });
    renderAll();
    operationFinish('Importação concluída', 'Backup importado sem perder a cópia local anterior.');
}
catch {
    operationFinish('Não foi possível importar', 'O arquivo selecionado não é um backup válido.', true);
} e.target.value = ''; };
document.querySelector('#restorePreviousLocal').onclick = () => { const old = localStorage.getItem(PREVIOUS_KEY); if (!old) {
    alert('Ainda não existe uma cópia local anterior.');
    return;
} if (!confirm('Restaurar a cópia local anterior? O estado atual ficará como a próxima cópia de segurança.'))
    return; try {
    const current = JSON.stringify(state);
    state = migrateState(JSON.parse(old));
    localStorage.setItem(PREVIOUS_KEY, current);
    save({ backupOld: false });
    renderAll();
    alert('Cópia local restaurada.');
}
catch {
    alert('Não foi possível restaurar a cópia local.');
} };
document.querySelector('#resetData').onclick = () => { if (!confirm('Apagar os dados deste aparelho? Uma cópia local anterior será mantida.'))
    return; localStorage.setItem(PREVIOUS_KEY, JSON.stringify(state)); const keepCloud = { ...seed.cloud, ...state.cloud }; const keepSettings = { ...seed.settings, ...state.settings }; state = deepClone(seed); state.cloud = keepCloud; state.settings = keepSettings; state.importNote = false; save({ backupOld: false }); renderAll(); };
// Google Drive
function updateDriveStatus(message) { const el = document.querySelector('#driveStatus'), detail = document.querySelector('#driveDetail'); if (!el)
    return; const connected = !!(driveToken || cloudToken); el.textContent = connected ? 'Conectado' : 'Desconectado'; el.classList.toggle('ok', connected); if (message)
    detail.textContent = message;
else if (state.cloud.lastBackupAt)
    detail.textContent = `Último backup conhecido: ${new Date(state.cloud.lastBackupAt).toLocaleString('pt-BR')}. Retenção: ${state.cloud.retentionDays || 30} dias.`;
else
    detail.innerHTML = 'O backup será salvo na pasta configurada.'; }
function setBackupProgress(step, title, detail) { const root = document.querySelector('#backupProgress'); if (!root)
    return; const stages = { preparing: 12, folder: 32, uploading: 72, cleaning: 88, done: 100, error: 100 }; const percent = stages[step] || 0; root.classList.remove('hidden', 'done'); if (step === 'done')
    root.classList.add('done'); if (step === 'error')
    root.classList.add('done'); document.querySelector('#backupProgressTitle').textContent = title; document.querySelector('#backupProgressDetail').textContent = detail; document.querySelector('#backupProgressPercent').textContent = `${percent}%`; document.querySelector('#backupProgressBar').style.width = `${percent}%`; document.querySelector('#backupProgressIcon').textContent = step === 'done' ? '✓' : step === 'error' ? '!' : '↗'; }
function setDriveBusy(busy, label = 'Enviando backup…') { const button = document.querySelector('#backupDrive'); if (!button)
    return; button.disabled = busy; button.classList.toggle('busy', busy); button.textContent = busy ? label : 'Enviar backup agora'; if (!busy && label === 'Enviando backup…')
    button.textContent = 'Enviar backup agora'; }
...
async function connectCloudProvider() {
    ...
    if (provider === 'google') {
        ...
        updateDriveStatus('Google Drive conectado. A pasta existente foi validada.');
        return;
    }
    const config = oauthConfig[provider], clientId = state.cloud.providerClientId;
    if (!clientId)
        throw new Error(`Informe o Client ID do ${cloudProviderNames[provider]} antes de conectar.`);
    ...
    const params = new URLSearchParams({ client_id: clientId, response_type: 'code', redirect_uri: redirectUri, state: oauthState, code_challenge: challenge, code_challenge_method: 'S256' });
    if (config.scope)
        params.set('scope', config.scope);
    if (provider === 'dropbox')
        params.set('token_access_type', 'offline');
    const popup = window.open(`${config.authorize}?${params}`, 'saldoPlanOAuth', 'width=520,height=700');
    if (!popup)
        throw new Error('Permita pop-ups para concluir a conexão.');
    await new Promise((resolve, reject) => { const timer = setTimeout(() => { window.removeEventListener('message', receive); reject(new Error('A autorização expirou ou foi cancelada.')); }, 180000); function receive(event) { if (event.origin !== location.origin || event.data?.type !== 'saldoPlanOAuth')
        return; clearTimeout(timer); window.removeEventListener('message', receive); if (event.data.error) {
        reject(new Error(event.data.error));
        return;
    } fetch(config.token, { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams({ client_id: clientId, grant_type: 'authorization_code', code: event.data.code, redirect_uri: redirectUri, code_verifier: verifier }).toString() }).then(r => r.json()).then(data => { if (!data.access_token)
        throw new Error(data.error_description || 'Não foi possível obter o token.'); cloudToken = data.access_token; resolve(); }).catch(reject); } window.addEventListener('message', receive); });
    updateDriveStatus(`${cloudProviderNames[provider]} conectado. Backup pronto para uso.`);
}
function handleCloudOAuthCallback() { const params = new URLSearchParams(location.search), oauth = sessionStorage.getItem('saldoPlan.oauth'); if (!oauth || (!params.get('code') && !params.get('error')) || !window.opener)
    return false; window.opener.postMessage({ type: 'saldoPlanOAuth', code: params.get('code'), error: params.get('error_description') || params.get('error') }, location.origin); window.close(); return true; }
function requestDriveToken() { return new Promise((resolve, reject) => { const clientId = (document.querySelector('#googleClientId').value.trim() || state.cloud.clientId || ''); if (!clientId) {
    reject(new Error('Informe o Google OAuth Client ID em Dados e backup.'));
    return;
} if (!window.google?.accounts?.oauth2) {
    reject(new Error('O login do Google ainda não carregou. Tente novamente em alguns segundos.'));
    return;
} state.cloud.clientId = clientId; state.cloud.emailHint = document.querySelector('#googleEmailHint').value.trim() || state.cloud.emailHint; state.cloud.folderName = document.querySelector('#driveFolderName').value.trim() || state.cloud.folderName; state.cloud.autoBackup = document.querySelector('#driveAutoBackup').checked; save({ cloud: false }); const tokenClient = google.accounts.oauth2.initTokenClient({ client_id: clientId, scope: 'https://www.googleapis.com/auth/drive.file', login_hint: state.cloud.emailHint || undefined, callback: resp => { if (resp.error) {
        reject(new Error(resp.error));
        return;
    } driveToken = resp.access_token; updateDriveStatus('Google Drive conectado nesta sessão.'); resolve(driveToken); } }); tokenClient.requestAccessToken({ prompt: '' }); }); }
document.querySelector('#connectDrive').onclick = async () => { try {
...
async function driveFetch(url, options = {}) { if (!driveToken)
    throw new Error('Conecte o Google Drive primeiro.'); const headers = new Headers(options.headers || {}); headers.set('Authorization', `Bearer ${driveToken}`); const r = await fetch(url, { ...options, headers }); if (r.status === 401) {
    driveToken = null;
    updateDriveStatus('A autorização expirou. Conecte novamente.');
    throw new Error('Autorização do Google expirada.');
} if (!r.ok) {
    let msg = 'Erro no Google Drive.';
    try {
...
async function findOrCreateDriveFolder() {
    ...
    if (j.files?.[0]) {
        state.cloud.folderId = j.files[0].id;
        save({ backupOld: false, cloud: false });
        return state.cloud.folderId;
    }
    r = await driveFetch('https://www.googleapis.com/drive/v3/files?fields=id,name', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, mimeType: 'application/vnd.google-apps.folder' }) });
    j = await r.json();
    state.cloud.folderId = j.id;
    save({ backupOld: false, cloud: false });
    return j.id;
}
...
async function backupToOtherCloud() {
    if (!cloudToken)
        throw new Error(`Conecte ao ${cloudProviderNames[state.cloud.provider]} primeiro.`);
    ...
    if (state.cloud.provider === 'onedrive') {
        ...
        const r = await fetch(`${root}${encodeURIComponent(folder)}/${encodeURIComponent(name)}:/content`, { method: 'PUT', headers: { Authorization: `Bearer ${cloudToken}`, 'Content-Type': 'application/json' }, body });
        if (!r.ok)
            throw new Error('Não foi possível enviar o backup ao OneDrive.');
    }
        ...
        const r = await fetch('https://content.dropboxapi.com/2/files/upload', { method: 'POST', headers: { Authorization: `Bearer ${cloudToken}`, 'Content-Type': 'application/octet-stream', 'Dropbox-API-Arg': JSON.stringify({ path: `/${folder}/${name}`, mode: 'add', autorename: true, mute: false }) }, body });
        if (!r.ok)
            throw new Error('Não foi possível enviar o backup ao Dropbox.');
    }
    state.cloud.lastBackupAt = new Date().toISOString();
    save({ backupOld: false, cloud: false });
    updateDriveStatus(`Backup enviado ao ${cloudProviderNames[state.cloud.provider]}.`);
    alert('Backup enviado com sucesso.');
}
async function backupToDrive(silent = false) {
    if (state.cloud.provider !== 'google') {
        if (silent)
            return;
        if (!silent)
            operationStart('Enviando backup', 'Conectando ao armazenamento escolhido…');
        setDriveBusy(true, 'Enviando backup…');
        try {
            await backupToOtherCloud();
            if (!silent)
                operationFinish('Backup concluído', 'Suas informações foram salvas com sucesso.');
        }
        catch (e) {
            if (!silent)
                operationFinish('Backup não concluído', e.message, true);
            throw e;
        }
        finally {
            setDriveBusy(false);
    ...
    if (!silent) {
        operationStart('Backup em andamento', 'Organizando os dados deste aparelho…');
        setDriveBusy(true, 'Enviando backup…');
        setBackupProgress('preparing', 'Preparando backup', 'Organizando os dados deste aparelho…');
    }
    try {
        ...
        if (!silent) {
            ...
            document.querySelector('#operationDetail').textContent = 'A pasta existente foi validada com segurança.';
        }
        ...
        if (silent) {
            name = `saldoplan-backup-${stamp}.json`;
            file = null;
        }
        if (!file) {
        ...
        if (!silent)
            setBackupProgress('uploading', 'Enviando backup', 'Transferindo suas informações para o Google Drive…');
        ...
        if (silent)
            ...
            setBackupProgress('cleaning', 'Finalizando backup', 'Conferindo a retenção dos backups antigos…');
        state.cloud.lastBackupAt = now.toISOString();
        state.cloud.lastFileId = file.id;
        save({ backupOld: false, cloud: false });
        updateDriveStatus(`Backup enviado ao Drive em ${now.toLocaleString('pt-BR')}.`);
        if (!silent) {
            setBackupProgress('done', 'Backup concluído', 'Suas informações foram salvas com sucesso.');
            operationFinish('Backup concluído', 'Suas informações foram salvas com sucesso.');
        }
    }
    catch (e) {
        if (!silent) {
            setBackupProgress('error', 'Não foi possível concluir', 'Verifique a conexão e tente novamente.');
            operationFinish('Backup não concluído', e.message, true);
        }
        throw e;
    }
...
document.querySelector('#restoreDrive').onclick = async () => { if (state.cloud.provider !== 'google') {
    alert(`A restauração automática do ${cloudProviderNames[state.cloud.provider]} ainda será adicionada. Use Importar backup JSON por enquanto.`);
    return;
} try {
    if (!driveToken)
        await requestDriveToken();
    const folderId = await findOrCreateDriveFolder(), files = await listBackupFiles(folderId), file = files[0] || null;
    if (!file) {
        alert('Nenhum backup do SaldoPlan foi encontrado nessa pasta.');
        return;
    }
    if (!confirm(`Restaurar o backup do Drive${file.modifiedTime ? ` de ${new Date(file.modifiedTime).toLocaleString('pt-BR')}` : ''}? O estado atual será mantido como cópia local anterior.`))
        return;
    const r = await driveFetch(`https://www.googleapis.com/drive/v3/files/${file.id}?alt=media`), data = migrateState(await r.json());
    localStorage.setItem(PREVIOUS_KEY, JSON.stringify(state));
    state = data;
    state.cloud = { ...seed.cloud, ...state.cloud, clientId: document.querySelector('#googleClientId').value.trim() || state.cloud.clientId, emailHint: document.querySelector('#googleEmailHint').value.trim() || state.cloud.emailHint };
    save({ backupOld: false, cloud: false });
    renderAll();
...
document.querySelector('#installBtn').onclick = async () => { if (!deferredPrompt)
    return; deferredPrompt.prompt(); await deferredPrompt.userChoice; deferredPrompt = null; document.querySelector('#installBtn').classList.add('hidden'); };
if ('serviceWorker' in navigator && location.protocol.startsWith('http'))
    navigator.serviceWorker.register('sw.js').catch(() => { });
if (handleCloudOAuthCallback()) { }
else {
    applyTheme();
    renderAll();
}
